...
static void FBGRAddIfExists(NSMutableArray<NSString *> *paths, NSString *path) {
...
static void FBGRCollectMetadataFiles(NSMutableArray<NSString *> *paths, NSString *dir, NSUInteger depth) {
    ...
    for (NSString *name in items) {
        ...
        BOOL childDir = NO;
        [fm fileExistsAtPath:p isDirectory:&childDir];
        NSString *low = name.lowercaseString;
        if ([low hasPrefix:@"reactmobileconfigmetadata"] &&
            ([low hasSuffix:@".json"] || [low hasSuffix:@".json.gz"])) {
            FBGRAddIfExists(paths, p);
        }
        if (childDir && depth < 3) {
            if ([low containsString:@"fbtweaks"] || [low containsString:@"mobileconfig"] || [low containsString:@"runtime"] || [low containsString:@"config"]) {
                FBGRCollectMetadataFiles(paths, p, depth + 1);
            }
        }
    }
}
...
static NSData *FBGRLoadCatalogData(NSString **sourceOut) {
    ...
    // 0) App bundle first. In the sideloaded Facebook IPA the live metadata is
    // stored directly inside Facebook.app, for example:
    // /private/var/containers/Bundle/Application/<UUID>/Facebook.app/ReactMobileConfigMetadata.json
    NSString *bundlePath = NSBundle.mainBundle.bundlePath;
    if (bundlePath.length) {
        FBGRAddIfExists(paths, [bundlePath stringByAppendingPathComponent:@"ReactMobileConfigMetadata.json"]);
        FBGRAddIfExists(paths, [bundlePath stringByAppendingPathComponent:@"ReactMobileConfigMetadata.json.gz"]);
        FBGRCollectMetadataFiles(paths, bundlePath, 0);
    }

    // Exact path seen in the current test device/app install. This is kept only
    // as a highest-priority explicit candidate; the dynamic bundlePath above is
    // the stable path for future installs with a different UUID.
    FBGRAddIfExists(paths, @"/private/var/containers/Bundle/Application/5C38EEAB-1818-4C68-BF7D-A13378A902C2/Facebook.app/ReactMobileConfigMetadata.json");
    FBGRAddIfExists(paths, @"/private/var/containers/Bundle/Application/5C38EEAB-1818-4C68-BF7D-A13378A902C2/Facebook.app/ReactMobileConfigMetadata.json.gz");

    // 1) App data container first. This lets a sideloaded app refresh the catalog
    // by dropping ReactMobileConfigMetadata*.json(.gz) into Documents/FBTweaks,
    // Library/Application Support/FBTweaks, Caches/FBTweaks or tmp/FBTweaks.
    NSString *home = NSHomeDirectory();
    if (home.length) {
        for (NSString *rel in @[
            @"Documents/FBTweaks/runtime", @"Documents/FBTweaks", @"Documents",
            @"Library/Application Support/FBTweaks/runtime", @"Library/Application Support/FBTweaks",
            @"Library/Caches/FBTweaks/runtime", @"Library/Caches/FBTweaks",
            @"tmp/FBTweaks/runtime", @"tmp/FBTweaks",
        ]) {
            NSString *dir = [home stringByAppendingPathComponent:rel];
            FBGRAddIfExists(paths, [dir stringByAppendingPathComponent:@"ReactMobileConfigMetadata.json"]);
            FBGRAddIfExists(paths, [dir stringByAppendingPathComponent:@"ReactMobileConfigMetadata.json.gz"]);
            FBGRCollectMetadataFiles(paths, dir, 0);
        }
    }

    // 2) Shared app-support locations used by deb/rootless builds.
    ...
    for (NSString *base in bases) {
        FBGRAddIfExists(paths, [base stringByAppendingPathComponent:@"ReactMobileConfigMetadata.json"]);
        FBGRAddIfExists(paths, [base stringByAppendingPathComponent:@"ReactMobileConfigMetadata.json.gz"]);
        FBGRCollectMetadataFiles(paths, base, 0);
    }

    // 3) Dylib-relative resources for IPA/sideload injection.
    NSString *dylibDir = FBGRDylibDirectory();
    if (dylibDir.length) {
        for (NSString *rel in @[
            @"ReactMobileConfigMetadata.json", @"ReactMobileConfigMetadata.json.gz",
            @"resources/runtime/ReactMobileConfigMetadata.json", @"resources/runtime/ReactMobileConfigMetadata.json.gz",
            @"../resources/runtime/ReactMobileConfigMetadata.json", @"../resources/runtime/ReactMobileConfigMetadata.json.gz",
        ]) FBGRAddIfExists(paths, [dylibDir stringByAppendingPathComponent:rel]);
        FBGRCollectMetadataFiles(paths, dylibDir, 0);
    }

    // 4) Main app bundle resources.
    NSBundle *main = NSBundle.mainBundle;
    NSString *p = [main pathForResource:@"ReactMobileConfigMetadata" ofType:@"json"];
    if (p) FBGRAddIfExists(paths, p);
    p = [main pathForResource:@"ReactMobileConfigMetadata" ofType:@"json.gz"];
    if (p) FBGRAddIfExists(paths, p);
    FBGRCollectMetadataFiles(paths, main.resourcePath, 0);
    ...
    if (embedded.length) {
        if (sourceOut) *sourceOut = [NSString stringWithFormat:@"embedded ReactMobileConfigMetadata (%lu bytes compressed)", (unsigned long)FBGRMCEmbeddedCatalogCompressedSize()];
        return embedded;
    }
    return nil;
}
