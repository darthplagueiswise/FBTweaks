...
static void FBTMobileConfigDebugWarmup(void) {
    // The native-UI assist is itself an explicit opt-in/defaulted preference.
    // Install the safe fishhook/context runtime even when the separate monitor
    // row is off, so the controller does not race its first parameter tap.
