...
// The advanced native MobileConfig UI lives in FBRarelyUsedFramework and can
// be loaded well after the tweak constructor. These hooks do not suppress the
// native fetch error and do not fabricate QE metadata. They install/refresh
// the local readers and context capture only when the native UI is alive.
...
static void fbt_FBMobileConfigDebug_selectParam(id self,
    ...
    if (orig_FBMobileConfigDebug_selectParam) {
        orig_FBMobileConfigDebug_selectParam(self,
                                             _cmd,
                                             param,
                                             key,
                                             configName,
...
void FBTInstallMobileConfigDebugUIHooks(void) {
    ...
    if (!orig_FBMobileConfigDebug_selectParam &&
        FBTMCUIEncodingEquals(cls,
                              selectSel,
                              "v52@0:8r^v16i24r^v28q36@44")) {
        MSHookMessageEx(cls,
                        selectSel,
