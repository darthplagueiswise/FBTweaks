// FBTRuntimeXrefScanner.m
#import "FBTRuntimeXrefScanner.h"
...
#define XLOG(fmt, ...) os_log(OS_LOG_DEFAULT, "[FBTGate] Xref " fmt, ##__VA_ARGS__)

@implementation FBTXrefHit
@end

#pragma mark - ARM64 decoders

static inline int fbt_decode_adrp(uint32_t instr, uint64_t pc, int *rd, uint64_t *page) {
...
static inline int fbt_decode_adr(uint32_t instr, uint64_t pc, int *rd, uint64_t *addr) {
...
static inline int fbt_decode_add_imm(uint32_t instr, int *rd, int *rn, uint64_t *imm) {
...
static inline int fbt_decode_ldr_imm64(uint32_t instr, int *rt, int *rn, uint64_t *imm) {
...
static inline int fbt_decode_bl(uint32_t instr, uint64_t pc, uint64_t *target) {
...
static inline int fbt_decode_blr(uint32_t instr, int *rn) {
    if ((instr & 0xFFFFFC1Fu) != 0xD63F0000u) return 0;
    *rn = (int)((instr >> 5) & 0x1F);
    return 1;
}

static inline int fbt_decode_br(uint32_t instr, int *rn) {
...
static BOOL fbt_is_scope_image(const char *path) {
    if (!path) return NO;
    const char *slash = strrchr(path, '/');
    const char *base = slash ? slash + 1 : path;
    return strcmp(base, "Facebook") == 0
        || strstr(path, "/FBSharedFramework") != NULL
        || strstr(path, "/FBSharedDynamicFramework") != NULL
        || strstr(path, "/FBReactNativeProductsFramework") != NULL;
}

static NSString *fbt_short_image_name(const char *path) {
    if (!path) return @"?";
    const char *slash = strrchr(path, '/');
    return [NSString stringWithUTF8String:slash ? slash + 1 : path];
}

static BOOL fbt_text_bounds_for_index(uint32_t i, const uint32_t **outCode, uint64_t *outBase, size_t *outCount, NSString **outName) {
    ...
    if (outName) *outName = fbt_short_image_name(path);
    return YES;
}

static BOOL fbt_section_bounds_for_index(uint32_t i, const char *segname, const char *sectname, const uint32_t **outCode, uint64_t *outBase, size_t *outCount) {
    const struct mach_header *raw = _dyld_get_image_header(i);
...
static BOOL fbt_address_is_in_text(uintptr_t address) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!fbt_text_bounds_for_index(i, &code, &base, &count, NULL)) return NO;
...
static BOOL fbt_safe_read_ptr(uint64_t address, uint64_t *outValue) {
...
static FBTXrefHit *fbt_make_hit(uint64_t loadPC, uint64_t callPC, uint64_t callee, NSString *image, NSString *kind) {
    FBTXrefHit *h = [FBTXrefHit new];
    ...
    h.detail = [NSString stringWithFormat:@"%@ · caller %@ · call %@ · load %@",
                kind ?: @"xref",
                h.callerSymbol ?: fbt_describe_address((uintptr_t)callPC),
                fbt_describe_address((uintptr_t)callPC),
                fbt_describe_address((uintptr_t)loadPC)];
    return h;
}


static NSSet<NSNumber *> *fbt_import_stub_aliases_for_target(uint32_t imageIndex, uint64_t target) {
    NSMutableSet<NSNumber *> *aliases = [NSMutableSet set];
    const char *sections[] = { "__stubs", "__auth_stubs", "__text" };
    for (unsigned si = 0; si < 3; si++) {
        const uint32_t *code = NULL; uint64_t base = 0; size_t count = 0;
        if (!fbt_section_bounds_for_index(imageIndex, "__TEXT", sections[si], &code, &base, &count)) continue;
        for (size_t i = 0; i + 2 < count; i++) {
            uint64_t pc = base + i * 4;
            int rd = -1; uint64_t page = 0;
            if (!fbt_decode_adrp(code[i], pc, &rd, &page)) continue;
            int lrt = -1, lrn = -1; uint64_t limm = 0;
            if (!fbt_decode_ldr_imm64(code[i + 1], &lrt, &lrn, &limm) || lrn != rd) continue;
            uint64_t slot = page + limm, pointed = 0;
            if (!fbt_safe_read_ptr(slot, &pointed) || pointed != target) continue;
            for (size_t j = i + 2; j < count && j < i + 7; j++) {
                int brn = -1;
                if ((fbt_decode_br(code[j], &brn) || fbt_decode_blr(code[j], &brn)) && brn == lrt) {
...
static NSArray<FBTXrefHit *> *fbt_scan_text(const uint32_t *code, uint64_t base, size_t count,
    ...
    NSMutableArray<FBTXrefHit *> *hits = [NSMutableArray array];
    if (!code || count < 2) return hits;
    uint64_t scanned = 0;
    uint64_t realBudget = budget ?: [FBTRuntimeXrefScanner defaultBudget];
    ...
    for (size_t i = 0; i + 1 < count; i++) {
        ...
        uint64_t blTarget = 0;
        if (targetIsCode && fbt_decode_bl(code[i], pc, &blTarget) && (blTarget == target || [codeAliases containsObject:@(blTarget)])) {
            ...
            if (![dedupe containsObject:key]) {
                ...
                FBTXrefHit *h = fbt_make_hit(pc, pc, target, image, viaStub ? @"direct-call-via-import-stub" : @"direct-call");
                if (viaStub) h.detail = [NSString stringWithFormat:@"direct-call-via-import-stub · stub %@ · caller %@", fbt_describe_address((uintptr_t)blTarget), h.callerSymbol ?: @"?"];
                [hits addObject:h];
            }
        ...
        int rd = -1; uint64_t page = 0;
        if (fbt_decode_adrp(code[i], pc, &rd, &page)) {
            int ard = -1, arn = -1; uint64_t aimm = 0;
            if (i + 1 < count && fbt_decode_add_imm(code[i + 1], &ard, &arn, &aimm) && arn == rd) {
                uint64_t addr = page + aimm;
                if (addr == target) { resolvedAddress = target; loadedReg = ard; kind = @"address-load"; }
            } else {
                int lrt = -1, lrn = -1; uint64_t limm = 0;
                if (i + 1 < count && fbt_decode_ldr_imm64(code[i + 1], &lrt, &lrn, &limm) && lrn == rd) {
                    uint64_t slot = page + limm;
                    uint64_t pointed = 0;
                    if (slot == target) { resolvedAddress = target; loadedReg = lrt; kind = @"slot-address-load"; }
                    else if (fbt_safe_read_ptr(slot, &pointed) && pointed == target) { resolvedAddress = target; loadedReg = lrt; kind = @"got-load"; }
                }
            }
        }
        ...
        if (!resolvedAddress) {
            ...
            if (fbt_decode_adr(code[i], pc, &adrReg, &adrAddr) && adrAddr == target) {
...
@implementation FBTRuntimeXrefScanner

+ (NSString *)describeAddress:(uintptr_t)address { return fbt_describe_address(address); }

+ (uint64_t)defaultBudget { return 9000000ULL; }

+ (NSArray<FBTXrefHit *> *)consumersOfAddress:(uintptr_t)targetAddress
    ...
    BOOL targetIsCode = fbt_address_is_in_text(targetAddress);
    NSMutableArray<FBTXrefHit *> *all = [NSMutableArray array];
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        } else if (!fbt_is_scope_image(path)) {
            continue;
        }
        const uint32_t *code = NULL; uint64_t base = 0; size_t count = 0; NSString *name = nil;
        if (!fbt_text_bounds_for_index(i, &code, &base, &count, &name)) continue;
        ...
        NSSet<NSNumber *> *aliases = targetIsCode ? fbt_import_stub_aliases_for_target(i, (uint64_t)targetAddress) : [NSSet set];
        NSArray *hits = fbt_scan_text(code, base, count, (uint64_t)targetAddress, targetIsCode, aliases,
...
+ (void)findConsumersOfAddress:(uintptr_t)targetAddress
                 imageSubstring:(NSString *)imageSubstring
                         budget:(uint64_t)budget
                        maxHits:(int)maxHits
                     completion:(void (^)(NSArray<FBTXrefHit *> *, BOOL))completion {
    if (!completion) return;
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        BOOL hitBudget = NO;
        NSArray<FBTXrefHit *> *hits = [self consumersOfAddress:targetAddress
