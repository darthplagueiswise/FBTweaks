...
static void HookClass(NSString *name) {
    ...
    SEL sels[] = { sel_registerName("isEnabled"), sel_registerName("isHomeFeedHeaderEnabled"), sel_registerName("isGlassRenderingOptimizationEnabled"), sel_registerName("isLegibilityBlurEnabled") };
...
extern "C" void FBGRLiquidGlassEnsureInstalled(void) {
    ...
    gInstalled = YES;
    FBGRLogAppend([NSString stringWithFormat:@"LiquidGlass helper hooks installed=%lu", (unsigned long)gHookN]);
}
extern "C" void FBGRLiquidGlassSetForced(BOOL forced) { gForce = forced; if (forced) FBGRLiquidGlassEnsureInstalled(); }
