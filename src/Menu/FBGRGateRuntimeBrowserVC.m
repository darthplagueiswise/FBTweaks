...
- (NSArray<FBGRMCParam*>*)overriddenParams {
    ...
    for (NSNumber *n in ids) {
        ...
        if (p) [r addObject:p];
            ...
            ph.type = @"boolValue"; ph.group = @"unknown"; ph.paramName = @"(slot desconhecido)"; ph.unitType = 0;
            [r addObject:ph];
        }
    }
    return r;
...
- (void)tableView:(UITableView *)tv didSelectRowAtIndexPath:(NSIndexPath *)ip {
    ...
    FBGRMCParam *p = self.visible[(NSUInteger)ip.row];
    if (p.isBool) return; // bool is handled by the switch

    // Non-bool params are READ-ONLY. We only override booleans because the hook
    // forces a BOOL return value; forcing a bool onto a string/int/double param
    // is meaningless and was a real bug (it wrote junk overrides). Show info.
    NSString *msg = [NSString stringWithFormat:
        @"%@\n\nTipo: %@\nGrupo: %@\nconfigKey: %llu\nparamId: %ld\nslotId: %llu\nunitType: %ld\n\nApenas params booleanos podem ser forçados.",
        p.fullKey, p.type, p.group,
        (unsigned long long)p.configKey, (long)p.paramId,
        (unsigned long long)p.slotId, (long)p.unitType];
    UIAlertController *a = [UIAlertController alertControllerWithTitle:@"Param read-only"
                                                              message:msg
...
- (void)swToggled:(UISwitch *)sw {
    ...
    if (!p.isBool) return; // never store override on non-bool
