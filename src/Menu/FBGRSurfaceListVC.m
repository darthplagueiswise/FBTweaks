...
- (NSString *)tableView:(UITableView *)tv titleForFooterInSection:(NSInteger)section { if(section==0)return [NSString stringWithFormat:@"Metadata: %lu BOOL flags", (unsigned long)[FBGRMCCatalog shared].boolParams.count]; if(section==1)return @"Busca, aplicar hooks e reiniciar ficam no topo dos browsers."; return nil; }
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip {
    UITableViewCell *c=[tv dequeueReusableCellWithIdentifier:@"cell"]; if(!c)c=[[UITableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"cell"];
    FBGRApplyGlassCell(c); c.accessoryType=UITableViewCellAccessoryDisclosureIndicator; c.imageView.image=nil; c.detailTextLabel.text=nil; c.textLabel.textColor=FBGRTextColor();
    if(ip.section==0){ FBGRFeatureCategory cat=(FBGRFeatureCategory)[FBGRCategories()[ip.row] integerValue]; FBGRApplyRootTextCell(c, FBGRFeatureCategoryTitle(cat), [NSString stringWithFormat:@"%lu flags", (unsigned long)[[FBGRMCCatalog shared] paramsForCategory:cat].count]); c.imageView.image=FBGRSymbol(FBGRFeatureCategoryIcon(cat), FBGRSecondaryTextColor()); }
    else if(ip.section==1){ NSArray *titles=@[@"MobileConfig Runtime Browser",@"Executable Bool Runtime",@"FBSharedFramework Bool Runtime"]; NSArray *subs=@[@"ReactMobileConfigMetadata real, patchável por slotId",@"Varre /Facebook.app/Facebook",@"Varre /FBSharedFramework.framework/FBSharedFramework"]; FBGRApplyRootTextCell(c, titles[ip.row], subs[ip.row]); c.imageView.image=FBGRSymbol(ip.row==0?@"slider.horizontal.3":@"cpu", FBGRSecondaryTextColor()); }
    else { NSArray *titles=@[@"Instalar hooks MobileConfig",@"Aplicar overrides persistidos",@"Forçar helpers LiquidGlass",@"Preset DogFood/Internal",@"Limpar todos overrides",@"Reiniciar Facebook"]; FBGRApplyRootTextCell(c, titles[ip.row], @""); c.imageView.image=FBGRSymbol(@[@"bolt",@"arrow.clockwise",@"sparkles",@"ladybug",@"trash",@"power"][ip.row], ip.row>=4?UIColor.systemRedColor:FBGRSecondaryTextColor()); if(ip.row>=4)c.textLabel.textColor=UIColor.systemRedColor; }
    return c;
}
...
void FBGRPresentMenu(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        nav.view.backgroundColor = FBGRBackgroundColor();
