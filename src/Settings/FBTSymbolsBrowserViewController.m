// FBTSymbolsBrowserViewController.m
#import "FBTSymbolsBrowserViewController.h"
#import "../FBTUtils.h"
#import "FBTBrowserCompat.h"
#import "../Runtime/FBTCSymbolStub.h"
#import "../Runtime/FBTCRuntimePatchResolver.h"
...
@interface FBTCSymbolEntry : NSObject
...
@end
@implementation FBTCSymbolEntry @end

@interface FBTCSymbolGroup : NSObject
@property (nonatomic, copy) NSString *title;
@property (nonatomic, copy) NSArray<FBTCSymbolEntry *> *entries;
@end
@implementation FBTCSymbolGroup @end


static NSDictionary<NSString *, id> *FBTCResolverInfoForEntry(FBTCSymbolEntry *e) {
    if (!e) return @{};
    return @{
        @"symbol": e.name ?: @"",
        @"image": e.image ?: @"",
...
static NSString *FBTCModeTitle(FBTCSymbolsBrowserMode mode) {
    (void)mode;
    return @"Unified Runtime Browser";
}

static NSString *fbtc_section_label(const struct section_64 *sec) {
...
static void fbtc_collect_sections(const struct mach_header_64 *mh, NSMutableArray<NSValue *> *sections, const struct symtab_command **symtab, const struct segment_command_64 **linkedit) {
...
static BOOL FBTCIsImageWanted(NSString *path) {
    return [path.lastPathComponent isEqualToString:@"Facebook"]
        || [path containsString:@"/FBSharedFramework"]
        || [path containsString:@"/FBSharedDynamicFramework"]
        || [path containsString:@"/FBReactNativeProductsFramework"];
}

static NSString *FBTCImageShortName(NSString *path) {
    if ([path containsString:@"/FBSharedDynamicFramework"]) return @"FBSharedDynamicFramework";
    if ([path containsString:@"/FBReactNativeProductsFramework"]) return @"FBReactNativeProductsFramework";
    if ([path containsString:@"/FBSharedFramework"]) return @"FBSharedFramework";
    if ([path.lastPathComponent isEqualToString:@"Facebook"]) return @"Facebook";
    return path.lastPathComponent ?: @"Image";
}

static void FBTCCollectImportedSymbolNamesForImage(uint32_t imageIndex, NSMutableSet<NSString *> *out) {
    const char *imageName = _dyld_get_image_name(imageIndex);
    if (!imageName || !out) return;
    NSString *path = [NSString stringWithUTF8String:imageName];
    if (!FBTCIsImageWanted(path)) return;
...
static NSSet<NSString *> *FBTCImportedSymbolNames(void) {
    static NSSet<NSString *> *names = nil;
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        NSMutableSet<NSString *> *set = [NSMutableSet set];
        uint32_t count = _dyld_image_count();
        for (uint32_t i = 0; i < count; i++) FBTCCollectImportedSymbolNamesForImage(i, set);
        names = set.copy;
    });
...
static BOOL FBTCRuntimeResolveSymbol(NSString *name, void **addrOut) {
...
static NSString *FBTCImageForAddress(uintptr_t address) {
...
static NSData *FBTCBytesAtAddress(uintptr_t address, NSUInteger maxLen) {
...
static NSString *FBTCHexDump(NSData *data, uintptr_t address) {
...
static int64_t FBTCSignExtend(int64_t v, unsigned bits) {
    int64_t m = 1LL << (bits - 1);
    return (v ^ m) - m;
}

static NSString *FBTCRegisterName(unsigned r, BOOL wide) {
    if (r == 31) return wide ? @"xzr/sp" : @"wzr/wsp";
    return [NSString stringWithFormat:@"%@%u", wide ? @"x" : @"w", r];
}

static NSString *FBTCDecodeARM64(uint32_t insn, uintptr_t pc) {
    ...
    if ((insn & 0xffc00000) == 0xa9000000 || (insn & 0xffc00000) == 0xa9800000 || (insn & 0xffc00000) == 0xa9400000) {
        ...
        return [NSString stringWithFormat:@"%@ %@, %@, [%@,#%d]%@", load?@"ldp":@"stp", FBTCRegisterName(rt, YES), FBTCRegisterName(rt2, YES), FBTCRegisterName(rn, YES), imm7 * 8, mode];
    }
    if ((insn & 0x7f800000) == 0x52800000) {
        unsigned rd = insn & 31;
        unsigned imm = (insn >> 5) & 0xffff;
        unsigned hw = (insn >> 21) & 3;
        return [NSString stringWithFormat:@"movz %@, #0x%x, lsl #%u", FBTCRegisterName(rd, NO), imm, hw * 16];
    }
    if ((insn & 0xfc000000) == 0x94000000) {
        int64_t imm = FBTCSignExtend((int64_t)(insn & 0x03ffffff), 26) << 2;
        uintptr_t dst = pc + imm;
        return [NSString stringWithFormat:@"bl %@", [FBTRuntimeXrefScanner describeAddress:dst]];
    }
    if ((insn & 0xfc000000) == 0x14000000) {
        int64_t imm = FBTCSignExtend((int64_t)(insn & 0x03ffffff), 26) << 2;
        return [NSString stringWithFormat:@"b %@", [FBTRuntimeXrefScanner describeAddress:(pc + imm)]];
    }
    if ((insn & 0xff000010) == 0x54000000) {
        int64_t imm = FBTCSignExtend((int64_t)((insn >> 5) & 0x7ffff), 19) << 2;
        unsigned cond = insn & 0xf;
        return [NSString stringWithFormat:@"b.%@ %@", cond < conds.count ? conds[cond] : @"?", [FBTRuntimeXrefScanner describeAddress:(pc + imm)]];
    }
    if ((insn & 0x9f000000) == 0x90000000) {
        unsigned rd = insn & 31;
        return [NSString stringWithFormat:@"adrp %@, <page>", FBTCRegisterName(rd, YES)];
    }
    if ((insn & 0x9f000000) == 0x10000000) {
        unsigned rd = insn & 31;
        return [NSString stringWithFormat:@"adr %@, <addr>", FBTCRegisterName(rd, YES)];
    }
    if ((insn & 0xffc00000) == 0x91000000) {
        unsigned rd = insn & 31, rn = (insn >> 5) & 31, imm = (insn >> 10) & 0xfff;
        return [NSString stringWithFormat:@"add %@, %@, #0x%x", FBTCRegisterName(rd, YES), FBTCRegisterName(rn, YES), imm];
    }
    if ((insn & 0xffc00000) == 0xd1000000) {
        unsigned rd = insn & 31, rn = (insn >> 5) & 31, imm = (insn >> 10) & 0xfff;
        return [NSString stringWithFormat:@"sub %@, %@, #0x%x", FBTCRegisterName(rd, YES), FBTCRegisterName(rn, YES), imm];
    }
    if ((insn & 0xffc00000) == 0xb1000000 || (insn & 0xffc00000) == 0xf1000000) {
        BOOL wide = (insn & 0x40000000) != 0;
        unsigned rd = insn & 31, rn = (insn >> 5) & 31, imm = (insn >> 10) & 0xfff;
        if (rd == 31) return [NSString stringWithFormat:@"cmn %@, #0x%x", FBTCRegisterName(rn, wide), imm];
        return [NSString stringWithFormat:@"adds %@, %@, #0x%x", FBTCRegisterName(rd, wide), FBTCRegisterName(rn, wide), imm];
    }
    if ((insn & 0xffc00000) == 0xb9400000) {
        unsigned rt = insn & 31, rn = (insn >> 5) & 31, imm = ((insn >> 10) & 0xfff) << 2;
        return [NSString stringWithFormat:@"ldr %@, [%@,#0x%x]", FBTCRegisterName(rt, NO), FBTCRegisterName(rn, YES), imm];
    }
    if ((insn & 0xffc00000) == 0xf9400000) {
        unsigned rt = insn & 31, rn = (insn >> 5) & 31, imm = ((insn >> 10) & 0xfff) << 3;
        return [NSString stringWithFormat:@"ldr %@, [%@,#0x%x]", FBTCRegisterName(rt, YES), FBTCRegisterName(rn, YES), imm];
    }
    if ((insn & 0xffe0ffe0) == 0xaa0003e0) {
        unsigned rd = insn & 31, rn = (insn >> 16) & 31;
        return [NSString stringWithFormat:@"mov %@, %@", FBTCRegisterName(rd, YES), FBTCRegisterName(rn, YES)];
    }
    return @"<decode pending>";
}

static NSString *FBTCDisassembleCommonARM64(uintptr_t address, NSUInteger instructionCount) {
    NSData *data = FBTCBytesAtAddress(address, instructionCount * 4);
    ...
    for (NSUInteger i = 0; i + 4 <= data.length; i += 4) {
        ...
        [out appendFormat:@"0x%llx  %08x  %@\n", (unsigned long long)pc, word, FBTCDecodeARM64(word, pc)];
    }
    return out.copy;
}

// ── Realtime resolver + patcher detail screen (v34) ─────────────────────────
@interface FBTCRealtimeDetailViewController : UIViewController
- (instancetype)initWithEntry:(FBTCSymbolEntry *)entry;
@end
...
typedef NS_ENUM(NSInteger, FBTCPatchStrategy) {
    FBTCPatchStrategyNone = 0,    // no sideload-safe patch (observe only)
    FBTCPatchStrategyObjC,        // MSHookMessageEx via FBTSymbolBrowserEngine
    FBTCPatchStrategyBoolStub,    // fishhook hardstub BOOL
    FBTCPatchStrategyTyped,       // fishhook typed force (int64/double/string)
    FBTCPatchStrategyParamDesc,   // reader-filter on IGMobileConfigBooleanValueForInternalUse
    FBTCPatchStrategyObserve,     // validated function observe hook, no value replacement
    FBTCPatchStrategyDataRebindString,
    FBTCPatchStrategyDataPatchBytes,
};

static FBTCPatchStrategy FBTCPatchStrategyFromRuntime(FBTCRuntimePatchStrategy s) {
    switch (s) {
        case FBTCRuntimePatchStrategyObjCBool: return FBTCPatchStrategyObjC;
        case FBTCRuntimePatchStrategyFunctionBool: return FBTCPatchStrategyBoolStub;
        case FBTCRuntimePatchStrategyFunctionTyped: return FBTCPatchStrategyTyped;
        case FBTCRuntimePatchStrategyFunctionObserve: return FBTCPatchStrategyObserve;
        case FBTCRuntimePatchStrategyDataReaderBool: return FBTCPatchStrategyParamDesc;
        case FBTCRuntimePatchStrategyDataRebindString: return FBTCPatchStrategyDataRebindString;
        case FBTCRuntimePatchStrategyDataPatchBytes: return FBTCPatchStrategyDataPatchBytes;
        default: return FBTCPatchStrategyNone;
    }
}

@interface FBTCRealtimeDetailViewController () <UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, strong) FBTCSymbolEntry *entry;
@property (nonatomic, strong) UITableView *table;
@property (nonatomic, assign) uintptr_t resolvedAddr;
@property (nonatomic, assign) FBTCPatchStrategy strategy;
...
- (void)refreshNow {
    void *runtime = NULL;
    FBTCRuntimeResolveSymbol(self.entry.name, &runtime);
    ...
    [rows addObject:@[@"Image", FBTCImageForAddress(self.resolvedAddr)]];
    ...
    FBTCRuntimePatchPlan *plan = [self currentRuntimePlan];
    ...
    if (self.entry.function && self.resolvedAddr) {
        self.disasmText = FBTCDisassembleCommonARM64(self.resolvedAddr, 14);
    } else if (self.resolvedAddr) {
        self.disasmText = FBTCHexDump(FBTCBytesAtAddress(self.resolvedAddr, 48), self.resolvedAddr);
    } else {
...
- (FBTCRuntimePatchPlan *)currentRuntimePlan {
    NSMutableDictionary *info = [FBTCResolverInfoForEntry(self.entry) mutableCopy];
    info[@"runtimeAddress"] = @(self.resolvedAddr ?: self.entry.address);
    info[@"symtabAddress"] = @(self.entry.address);
    return [FBTCRuntimePatchResolver resolvePlanForEntryInfo:info xrefHits:self.xrefHits];
}

- (FBTCPatchStrategy)resolveStrategy {
    FBTCRuntimePatchPlan *plan = [self currentRuntimePlan];
    ...
    return FBTCPatchStrategyFromRuntime(plan.strategy);
}
...
- (BOOL)currentlyApplied {
    return [FBTCRuntimePatchResolver isAppliedPlan:[self currentRuntimePlan]];
}
...
- (void)runXrefScan {
    ...
    [FBTRuntimeXrefScanner findConsumersOfAddress:self.resolvedAddr
                                    imageSubstring:nil
                                            budget:[FBTRuntimeXrefScanner defaultBudget]
                                           maxHits:12
                                        completion:^(NSArray<FBTXrefHit *> *hits, BOOL hitBudget) {
...
- (void)detailSwitchChanged:(UISwitch *)sender {
    FBTCRuntimePatchPlan *plan = [self currentRuntimePlan];
    if (sender.isOn) {
        if (plan.requiresPromptValue) { sender.on = [FBTCRuntimePatchResolver isEffectivelyEnabledForPlan:plan]; [self applyTapped]; return; }
        NSError *error = nil;
        if (![FBTCRuntimePatchResolver applyPlan:plan value:(plan.strategy == FBTCRuntimePatchStrategyFunctionObserve ? nil : @YES) error:&error]) {
            sender.on = [FBTCRuntimePatchResolver isEffectivelyEnabledForPlan:plan];
            [self showPatchError:error fallback:plan.reason];
        }
    } else {
        NSError *error = nil;
        id native = [FBTCRuntimePatchResolver currentNativeValueForPlan:plan];
        BOOL shouldForceOff = [native isKindOfClass:NSNumber.class] && [native boolValue] && (plan.strategy == FBTCRuntimePatchStrategyObjCBool || plan.strategy == FBTCRuntimePatchStrategyFunctionBool || plan.strategy == FBTCRuntimePatchStrategyDataReaderBool);
        BOOL ok = shouldForceOff ? [FBTCRuntimePatchResolver applyPlan:plan value:@NO error:&error] : [FBTCRuntimePatchResolver revertPlan:plan error:&error];
        if (!ok) { sender.on = [FBTCRuntimePatchResolver isEffectivelyEnabledForPlan:plan]; [self showPatchError:error fallback:plan.reason]; }
    }
    [self refreshNow];
}

- (void)showPatchError:(NSError *)error fallback:(NSString *)fallback {
    [FBTUtils showToastForDuration:1.5 title:@"Patch failed" subtitle:(error.localizedDescription ?: fallback ?: @"unknown error")];
}
...
- (void)applyTapped {
    ...
    switch (plan.strategy) {
        ...
        case FBTCRuntimePatchStrategyFunctionObserve:
            if (![FBTCRuntimePatchResolver applyPlan:plan value:nil error:&error]) { [self showPatchError:error fallback:plan.reason]; return; }
            break;
        default:
            if (![FBTCRuntimePatchResolver applyPlan:plan value:@YES error:&error]) { [self showPatchError:error fallback:plan.reason]; return; }
            break;
    }
    [FBTUtils showToastForDuration:1.0 title:@"Patch applied" subtitle:plan.symbol];
    [self refreshNow];
}
...
- (void)revertTapped {
    FBTCRuntimePatchPlan *plan = [self currentRuntimePlan];
    NSError *error = nil;
    if (![FBTCRuntimePatchResolver revertPlan:plan error:&error]) {
        [self showPatchError:error fallback:@"revert failed"];
        return;
    }
    [FBTUtils showToastForDuration:1.0 title:@"Reverted" subtitle:plan.symbol];
    [self refreshNow];
}

- (NSString *)promptMessageForTypedPlan:(FBTCRuntimePatchPlan *)plan kind:(NSString *)kind {
    ...
    id native = [FBTCRuntimePatchResolver currentNativeValueForPlan:plan];
    id forced = [FBTCRuntimePatchResolver currentForcedValueForPlan:plan];
    NSUInteger hits = [FBTCRuntimePatchResolver hitCountForPlan:plan];
...
- (void)promptTypedApply {
    ...
    NSString *kind = plan.returnKind ?: [FBTCSymbolStub returnKindForSymbol:n] ?: @"int64";
    ...
    [a addAction:[UIAlertAction actionWithTitle:@"Observe first" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *act) {
        __strong typeof(ws) ss = ws; if (!ss) return;
        NSError *error = nil;
        if (![FBTCRuntimePatchResolver applyPlan:plan value:nil error:&error]) [ss showPatchError:error fallback:plan.reason];
        else [FBTUtils showToastForDuration:1.2 title:@"Observe installed" subtitle:@"Use the feature path, then reopen this row."];
        [ss refreshNow];
    }]];
    [a addAction:[UIAlertAction actionWithTitle:@"Apply manual value" style:UIAlertActionStyleDestructive handler:^(__unused UIAlertAction *act) {
        ...
        FBTCRuntimePatchPlan *fresh = [ss currentRuntimePlan];
        if (![FBTCRuntimePatchResolver applyPlan:fresh value:value error:&error]) { [ss showPatchError:error fallback:fresh.reason]; return; }
        [FBTUtils showToastForDuration:1.0 title:@"Typed force applied" subtitle:n];
        [ss refreshNow];
    }]];
...
- (void)promptDataStringRebind {
    FBTCRuntimePatchPlan *plan = [self currentRuntimePlan];
    NSString *msg = [NSString stringWithFormat:@"%@\nSection: %@\nConsumer: %@\nCurrent override: %@\n\nUse only an NSString/key-compatible replacement. If there is no consumer/xref yet, observe or resolve xrefs before rebinding.", plan.symbol ?: @"", plan.section ?: @"?", plan.consumerSymbol ?: @"unconfirmed", [FBTCRuntimePatchResolver currentForcedValueForPlan:plan] ?: @"none"];
    ...
    [a addAction:[UIAlertAction actionWithTitle:@"Apply" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *act) {
        ...
        if (![FBTCRuntimePatchResolver applyPlan:plan value:value error:&error]) { [ss showPatchError:error fallback:plan.reason]; return; }
        [FBTUtils showToastForDuration:1.0 title:@"DATA pointer rebound" subtitle:plan.symbol];
        [ss refreshNow];
    }]];
    [self presentViewController:a animated:YES completion:nil];
}
...
- (void)promptDataPatchBytes {
    ...
    NSString *currentHex = [FBTCRuntimePatchResolver hexStringFromData:FBTCBytesAtAddress(plan.runtimeAddress ?: self.resolvedAddr, n)] ?: @"";
    ...
    [a addAction:[UIAlertAction actionWithTitle:@"Apply" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *act) {
        __strong typeof(ws) ss = ws; if (!ss) return;
        NSError *error = nil;
        NSData *bytes = [FBTCRuntimePatchResolver dataFromHexString:(a.textFields.firstObject.text ?: @"") error:&error];
        if (!bytes.length || ![FBTCRuntimePatchResolver applyPlan:plan value:bytes error:&error]) { [ss showPatchError:error fallback:plan.reason]; return; }
        [FBTUtils showToastForDuration:1.0 title:@"DATA bytes patched" subtitle:plan.symbol];
        [ss refreshNow];
    }]];
    [self presentViewController:a animated:YES completion:nil];
}
...
- (void)copyReport {
    ...
    if (self.xrefHits.count) {
        [s appendString:@"\nConsumers (xref):\n"];
        for (FBTXrefHit *h in self.xrefHits) [s appendFormat:@"  %@  (caller %@, load 0x%lx)\n", h.calleeSymbol ?: @"?", h.callerSymbol ?: @"?", (unsigned long)h.loadPC];
    }
    [s appendFormat:@"\n%@\n", self.disasmText ?: @""];
    UIPasteboard.generalPasteboard.string = s;
    [FBTUtils showToastForDuration:1.0 title:@"Report copied" subtitle:nil];
}
...
- (NSInteger)tableView:(UITableView *)tv numberOfRowsInSection:(NSInteger)s {
    ...
    if (s == 1) {
        if (self.strategy == FBTCPatchStrategyNone) return 1;          // reason
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip {
    ...
    if (ip.section == 1) {
        FBTCRuntimePatchPlan *plan = [self currentRuntimePlan];
        if (self.strategy == FBTCPatchStrategyNone) {
        ...
        if (ip.row == 1) {
            ...
            sw.on = [FBTCRuntimePatchResolver isEffectivelyEnabledForPlan:plan];
            sw.enabled = !plan.requiresPromptValue || [FBTCRuntimePatchResolver isAppliedPlan:plan];
        ...
        if (ip.row == 2) {
            ...
            if (self.strategy == FBTCPatchStrategyTyped) cell.textLabel.text = applied ? @"Set forced value… (re-apply)" : @"Set forced value… (apply)";
            else if (self.strategy == FBTCPatchStrategyDataRebindString) cell.textLabel.text = applied ? @"Set replacement string… (re-apply)" : @"Set replacement string…";
            else if (self.strategy == FBTCPatchStrategyDataPatchBytes) cell.textLabel.text = applied ? @"Set patch bytes… (re-apply)" : @"Set patch bytes…";
            else if (self.strategy == FBTCPatchStrategyObserve) cell.textLabel.text = applied ? @"Observing ✓ (tap to refresh)" : @"Install observe hook";
        ...
        cell.detailTextLabel.text = [FBTCRuntimePatchResolver stateSummaryForPlan:plan];
        return cell;
    }
    if (ip.section == 2) {
        ...
        FBTXrefHit *h = self.xrefHits[ip.row];
        cell.textLabel.text = h.calleeSymbol ?: [FBTRuntimeXrefScanner describeAddress:h.calleeAddress];
        cell.detailTextLabel.text = h.detail ?: [NSString stringWithFormat:@"%@ · caller %@ · load %@ · %@", h.referenceKind ?: @"xref", h.callerSymbol ?: @"?", [FBTRuntimeXrefScanner describeAddress:h.loadPC], h.image ?: @"?"];
        return cell;
    }
...
- (void)tableView:(UITableView *)tv didSelectRowAtIndexPath:(NSIndexPath *)ip {
    ...
    if (ip.section == 1 && self.strategy != FBTCPatchStrategyNone) {
...
static BOOL FBTCNameLooksSwiftOrCXX(NSString *name) {
...
static NSString *FBTCABIForName(NSString *name, BOOL function, NSString *section) {
    if (!function) {
    ...
    if (FBTCNameLooksSwiftOrCXX(name)) return @"Swift/C++ direct symbol; disassemble/xref first, sideload hook not assumed";
    return @"unknown function ABI; classify before hook";
}

static NSString *FBTCHookPlanForName(NSString *name, BOOL function, NSString *section) {
    if (!function) {
    ...
    if ([FBTCSymbolStub isForceableSymbol:name]) return @"Hardstub BOOL allowed: mov w0,#1; ret via fishhook";
...
static NSArray<NSString *> *FBTCDefaultFiltersForMode(FBTCSymbolsBrowserMode mode) {
    if (mode == FBTCSymbolsBrowserModeObjCMethods) return @[@"MobileConfig", @"Gating", @"Employee", @"Dogfood", @"Internal", @"Debug", @"Plus", @"IGDS", @"Launcher", @"SUBSBenefit"];
    if (mode == FBTCSymbolsBrowserModeDataParams) return @[@"ig_is_employee", @"ig_user_session", @"xav_switcher", @"mc_team", @"MapFields", @"MapSchema", @"OpenSettings", @"DeveloperAccount", @"FeatureFlags"];
    if (mode == FBTCSymbolsBrowserModeSwiftDisassembly) return @[@"$s", @"_Tt", @"ConsumerSubs", @"SUBSBenefit", @"MobileConfig", @"Dogfood", @"Eligibility", @"FeatureFlags"];
    return @[@"MobileConfig", @"EasyGating", @"MSGC", @"MCI", @"TALEvents", @"RegisterMappings", @"UpdateConfigs", @"SetConfigOverrides", @"InternalApps", @"Minos"];
}


static NSUInteger FBTCKnownDataSizeForSymbol(const struct nlist_64 *nl, uint32_t nsyms, uint32_t idx) {
...
static void FBTCEnumerateImageSymbolsAtIndex(uint32_t imageIndex, NSMutableArray<FBTCSymbolEntry *> *out) {
    const char *imageName = _dyld_get_image_name(imageIndex);
    if (!imageName) return;
    NSString *path = [NSString stringWithUTF8String:imageName];
    if (!FBTCIsImageWanted(path)) return;
    ...
    const struct symtab_command *symtab = NULL;
    const struct segment_command_64 *linkedit = NULL;
    fbtc_collect_sections(mh, sections, &symtab, &linkedit);
    ...
    for (uint32_t i = 0; i < symtab->nsyms; i++) {
        ...
        NSString *section = fbtc_section_label(sec);
        BOOL isText = sec && strncmp(sec->segname, "__TEXT", 16) == 0 && strncmp(sec->sectname, "__text", 16) == 0;
        FBTCSymbolEntry *e = [FBTCSymbolEntry new];
        e.name = name;
        e.image = FBTCImageShortName(path);
        e.section = section;
        e.function = isText;
        e.data = !isText;
        e.swiftLike = FBTCNameLooksSwiftOrCXX(name);
        e.address = (uintptr_t)nl[i].n_value + (uintptr_t)slide;
        e.dataSize = isText ? 0 : FBTCKnownDataSizeForSymbol(nl, symtab->nsyms, i);
        e.kind = isText ? (e.swiftLike ? @"Swift/C++ function" : @"C/function") : @"DATA/const";
        e.abi = FBTCABIForName(name, isText, section);
        e.hookPlan = FBTCHookPlanForName(name, isText, section);
        e.resolvable = (dlsym(RTLD_DEFAULT, name.UTF8String) != NULL || dlsym(RTLD_DEFAULT, [[@"_" stringByAppendingString:name] UTF8String]) != NULL);
        e.hasBindPointer = [FBTCImportedSymbolNames() containsObject:name];
        [out addObject:e];
    }
}


static NSArray<FBTCSymbolEntry *> *FBTCEnumerateObjCEntriesForImage(FBTSymbolImage image) {
    NSMutableArray<FBTCSymbolEntry *> *out = [NSMutableArray array];
    NSString *imgName = @"Facebook";
    if (image == FBTSymbolImageFBShared) imgName = @"FBSharedFramework";
    else if (image == FBTSymbolImageReactNative) imgName = @"FBReactNativeProductsFramework";
    else if (image == FBTSymbolImageFBSharedDynamic) imgName = @"FBSharedDynamicFramework";
    NSArray<FBTSymbolClass *> *classes = [FBTSymbolBrowserEngine classesForImage:image] ?: @[];
    for (FBTSymbolClass *cls in classes) {
        for (FBTSymbolGetter *g in cls.getters ?: @[]) {
            FBTCSymbolEntry *e = [FBTCSymbolEntry new];
            ...
            e.resolvable = YES;
            e.hasBindPointer = NO;
            e.abi = @"ObjC dispatch BOOL getter: id self, SEL _cmd -> BOOL w0; hook via FBTSymbolBrowserEngine/MSHookMessageEx.";
...
static NSArray<FBTCSymbolEntry *> *FBTCEnumerateInstagramAndFBSharedSymbols(void) {
    NSMutableArray *out = [NSMutableArray array];
    uint32_t count = _dyld_image_count();
    for (uint32_t i = 0; i < count; i++) FBTCEnumerateImageSymbolsAtIndex(i, out);
    [out sortUsingComparator:^NSComparisonResult(FBTCSymbolEntry *a, FBTCSymbolEntry *b) {
...
static char kFBTCSymbolRowPayloadKey;
static char kFBTCSymbolSwitchPayloadKey;
static char kFBTCSymbolGroupPayloadKey;

@interface FBTSymbolsBrowserViewController () <UISearchResultsUpdating>
@end

@implementation FBTSymbolsBrowserViewController {
    FBTCSymbolsBrowserMode _mode;
    NSArray<FBTCSymbolEntry *> *_allSymbols;
    NSString *_query;
...
- (instancetype)init { return [self initWithMode:FBTCSymbolsBrowserModeObjCMethods]; }
- (instancetype)initWithMode:(FBTCSymbolsBrowserMode)mode {
    self = [super initWithTitle:FBTCModeTitle(mode)];
...
- (void)viewDidLoad {
    ...
    FBTUIKit26ConfigureSearchNavigationItem(self.navigationItem);
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        NSMutableArray *symbols = [FBTCEnumerateInstagramAndFBSharedSymbols() mutableCopy];
        [symbols addObjectsFromArray:FBTCEnumerateObjCEntriesForImage(FBTSymbolImageFacebook)];
        [symbols addObjectsFromArray:FBTCEnumerateObjCEntriesForImage(FBTSymbolImageFBShared)];
        [symbols addObjectsFromArray:FBTCEnumerateObjCEntriesForImage(FBTSymbolImageReactNative)];
        [symbols addObjectsFromArray:FBTCEnumerateObjCEntriesForImage(FBTSymbolImageFBSharedDynamic)];
...
- (void)configureUnifiedTabs {
    _imageSegment = [[UISegmentedControl alloc] initWithItems:@[@"All", @"Exec", @"FBShared", @"RN", @"FBSharedDyn"]];
    ...
    FBTUIKit26ConfigureSegmentedControl(_imageSegment);
    ...
    FBTUIKit26ConfigureSegmentedControl(_kindSegment);
...
- (void)unifiedTabChanged:(__unused id)sender {
    _mode = (FBTCSymbolsBrowserMode)_kindSegment.selectedSegmentIndex;
    [self rebuildSections];
}
...
- (BOOL)entryMatchesMode:(FBTCSymbolEntry *)e {
    if (_imageSegment.selectedSegmentIndex == 1 && ![e.image isEqualToString:@"Facebook"]) return NO;
    if (_imageSegment.selectedSegmentIndex == 2 && ![e.image isEqualToString:@"FBSharedFramework"]) return NO;
    if (_imageSegment.selectedSegmentIndex == 3 && ![e.image isEqualToString:@"FBReactNativeProductsFramework"]) return NO;
    if (_imageSegment.selectedSegmentIndex == 4 && ![e.image isEqualToString:@"FBSharedDynamicFramework"]) return NO;
    if (_mode == FBTCSymbolsBrowserModeObjCMethods) return e.objcSelectorName.length > 0;
    if (_mode == FBTCSymbolsBrowserModeCFunctions) return e.function && !e.swiftLike && e.objcSelectorName.length == 0;
    if (_mode == FBTCSymbolsBrowserModeDataParams) return e.data;
    return e.function && e.swiftLike && e.objcSelectorName.length == 0;
}

- (BOOL)entry:(FBTCSymbolEntry *)e matchesTokens:(NSArray<NSString *> *)tokens {
...
- (BOOL)entryMatchesDefaultFilters:(FBTCSymbolEntry *)e {
    if (e.objcSelectorName.length) { NSString *k=[NSString stringWithFormat:@"%@%@", e.objcClassMethod?@"+":@"", [NSString stringWithFormat:@"%@#%@", e.objcClassName?:@"", e.objcSelectorName?:@""]]; if ([FBTSymbolBrowserEngine overrideForKey:k] != nil) return YES; }
    if ([FBTCRuntimePatchResolver persistedPatchForSymbol:e.name] != nil || [FBTCSymbolStub forceForSymbol:e.name] != nil || [FBTCSymbolStub typedForceForSymbol:e.name] != nil || [FBTCSymbolStub forceForParamDescriptorSymbol:e.name] != nil || [FBTCSymbolStub observeForParamDescriptorSymbol:e.name] || [FBTCSymbolStub observeForSymbol:e.name] || [FBTCSymbolStub hookInstalledForSymbol:e.name]) return YES;
    for (NSString *f in FBTCDefaultFiltersForMode(_mode)) if ([e.name.lowercaseString containsString:f.lowercaseString] || [e.abi.lowercaseString containsString:f.lowercaseString]) return YES;
    return NO;
}

- (NSString *)subtitleForEntry:(FBTCSymbolEntry *)e {
    FBTCRuntimePatchPlan *plan = [self resolverPlanForEntry:e];
    NSMutableArray *bits = [NSMutableArray array];
    [bits addObject:e.section ?: @"section?"];
    [bits addObject:plan.shortStrategyName ?: @"resolve only"];
    [bits addObject:[FBTCRuntimePatchResolver stateSummaryForPlan:plan]];
...
- (NSString *)detailForEntry:(FBTCSymbolEntry *)e {
...
- (void)pushRealtimeDetailForEntry:(FBTCSymbolEntry *)entry {
    if (!entry) return;
    [self.navigationController pushViewController:[[FBTCRealtimeDetailViewController alloc] initWithEntry:entry] animated:YES];
}



- (NSString *)objcOverrideKeyForEntry:(FBTCSymbolEntry *)entry {
...
- (BOOL)entryLooksParamDescriptor:(FBTCSymbolEntry *)entry {
    NSString *n = entry.name ?: @"";
    if ([FBTCSymbolStub isParamDescriptorSymbol:n]) return YES;
    if (!entry.data) return NO;
    if (!([n hasPrefix:@"ig_"] || [n hasPrefix:@"xav_"] || [n hasPrefix:@"mc_team_"])) return NO;
    return [FBTCSymbolStub canForceAsParamDescriptor:n];
}

- (FBTCRuntimePatchPlan *)resolverPlanForEntry:(FBTCSymbolEntry *)entry {
    return [FBTCRuntimePatchResolver resolvePlanForEntryInfo:FBTCResolverInfoForEntry(entry) xrefHits:nil];
}

- (FBTCPatchStrategy)inlineStrategyForEntry:(FBTCSymbolEntry *)entry {
    return FBTCPatchStrategyFromRuntime([self resolverPlanForEntry:entry].strategy);
}

- (BOOL)entryHasInlineToggle:(FBTCSymbolEntry *)entry {
    return [self resolverPlanForEntry:entry].inlineToggleSafe;
}

- (BOOL)entrySwitchValue:(FBTCSymbolEntry *)entry {
    return [FBTCRuntimePatchResolver isEffectivelyEnabledForPlan:[self resolverPlanForEntry:entry]];
}

- (NSString *)inlineStrategyShortLabelForEntry:(FBTCSymbolEntry *)entry {
    return [self resolverPlanForEntry:entry].shortStrategyName ?: @"resolve only";
}

- (NSString *)categoryForEntry:(FBTCSymbolEntry *)entry {
    NSString *n = entry.name ?: @"";
    ...
    if ([[FBTCSymbolStub returnKindForSymbol:n] isEqualToString:@"action"]) return @"Actions / registration";
    return @"Other validated C symbols";
}

- (UIView *)switchAccessoryForEntry:(FBTCSymbolEntry *)entry {
    UISwitch *sw = [UISwitch new];
    sw.on = [self entrySwitchValue:entry];
    sw.onTintColor = [FBTUtils FBTColor_Primary];
    objc_setAssociatedObject(sw, &kFBTCSymbolSwitchPayloadKey, entry, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)runtimePatchSwitchChanged:(UISwitch *)sender {
    FBTCSymbolEntry *entry = objc_getAssociatedObject(sender, &kFBTCSymbolSwitchPayloadKey);
    if (!entry) return;
    [self setEntry:entry enabled:sender.isOn sender:sender];
}

- (void)setEntry:(FBTCSymbolEntry *)entry enabled:(BOOL)enabled sender:(UISwitch *)sender {
    FBTCRuntimePatchPlan *plan = [self resolverPlanForEntry:entry];
    NSError *error = nil;
    if (!enabled) {
        id native = [FBTCRuntimePatchResolver currentNativeValueForPlan:plan];
        BOOL shouldForceOff = [native isKindOfClass:NSNumber.class] && [native boolValue] && (plan.strategy == FBTCRuntimePatchStrategyObjCBool || plan.strategy == FBTCRuntimePatchStrategyFunctionBool || plan.strategy == FBTCRuntimePatchStrategyDataReaderBool);
        ...
        if (!ok) {
            sender.on = [FBTCRuntimePatchResolver isEffectivelyEnabledForPlan:plan];
            [FBTUtils showToastForDuration:1.2 title:shouldForceOff ? @"Force OFF failed" : @"Revert failed" subtitle:error.localizedDescription ?: plan.symbol];
        }
        [self rebuildSections];
        return;
    }
    ...
    id value = (plan.strategy == FBTCRuntimePatchStrategyFunctionObserve) ? nil : @YES;
    if (![FBTCRuntimePatchResolver applyPlan:plan value:value error:&error]) {
        sender.on = NO;
        [FBTUtils showToastForDuration:1.2 title:@"Patch failed" subtitle:error.localizedDescription ?: plan.reason];
        [self rebuildSections];
        return;
    }
    [self rebuildSections];
...
- (void)setObjCEntry:(FBTCSymbolEntry *)entry force:(NSNumber *)value {
    if (!entry.objcSelectorName.length) return;
    FBTCRuntimePatchPlan *plan = [self resolverPlanForEntry:entry];
    NSError *error = nil;
    if (value) [FBTCRuntimePatchResolver applyPlan:plan value:value error:&error];
    else [FBTCRuntimePatchResolver revertPlan:plan error:&error];
    if (error) [FBTUtils showToastForDuration:1.2 title:@"Patch failed" subtitle:error.localizedDescription ?: plan.symbol];
    [self rebuildSections];
}

- (void)setParamEntry:(FBTCSymbolEntry *)entry force:(NSNumber *)value {
...
- (void)promptTypedForceForEntry:(FBTCSymbolEntry *)entry {
    NSString *kind = [FBTCSymbolStub returnKindForSymbol:entry.name] ?: @"unknown";
    if (![FBTCSymbolStub isTypedForceableSymbol:entry.name]) return;
    FBTCRuntimePatchPlan *plan = [self resolverPlanForEntry:entry];
    id native = [FBTCRuntimePatchResolver currentNativeValueForPlan:plan];
    id forced = [FBTCRuntimePatchResolver currentForcedValueForPlan:plan];
    NSString *msg = [NSString stringWithFormat:@"%@\nType: %@\nObserved native: %@\nCurrent override: %@\nHits: %lu\n\nUse Observe first if native is unknown. Manual values should come from an observed native value, a known enum/id, or a confirmed caller/xref.", entry.name ?: @"", kind, native ?: @"none yet", forced ?: @"none", (unsigned long)[FBTCRuntimePatchResolver hitCountForPlan:plan]];
    UIAlertController *a = [UIAlertController alertControllerWithTitle:[NSString stringWithFormat:@"Force %@", kind]
                                                                 message:msg
                                                          preferredStyle:UIAlertControllerStyleAlert];
    ...
    [a addAction:[UIAlertAction actionWithTitle:@"Observe first" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *act) {
        NSError *error = nil;
        FBTCRuntimePatchPlan *p = [weakSelf resolverPlanForEntry:entry];
        if (![FBTCRuntimePatchResolver applyPlan:p value:nil error:&error]) [FBTUtils showToastForDuration:1.2 title:@"Observe failed" subtitle:error.localizedDescription ?: p.reason];
        else [FBTUtils showToastForDuration:1.2 title:@"Observe installed" subtitle:@"Use the feature path, then reopen this row."];
    ...
    [a addAction:[UIAlertAction actionWithTitle:@"Apply manual value" style:UIAlertActionStyleDestructive handler:^(__unused UIAlertAction *act) {
        ...
        NSError *error = nil;
        FBTCRuntimePatchPlan *p = [weakSelf resolverPlanForEntry:entry];
        if (![FBTCRuntimePatchResolver applyPlan:p value:value error:&error]) {
            [FBTUtils showToastForDuration:1.2 title:@"Patch failed" subtitle:error.localizedDescription ?: p.reason];
            return;
        }
        [weakSelf rebuildSections];
    }]];
...
- (void)presentActionsForEntry:(FBTCSymbolEntry *)entry {
    if (!entry) return;
    ...
    if (entry.objcSelectorName.length) {
    ...
    } else if (entry.function && [FBTCSymbolStub isForceableSymbol:entry.name]) {
        [a addAction:[UIAlertAction actionWithTitle:@"Force BOOL YES (hardstub)" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *act){ [FBTCSymbolStub setForce:@YES forSymbol:entry.name]; [weakSelf rebuildSections]; }]];
        [a addAction:[UIAlertAction actionWithTitle:@"Force BOOL NO (hardstub)" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *act){ [FBTCSymbolStub setForce:@NO forSymbol:entry.name]; [weakSelf rebuildSections]; }]];
        [a addAction:[UIAlertAction actionWithTitle:@"Observe only" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *act){ [FBTCSymbolStub setObserve:YES forSymbol:entry.name]; [weakSelf rebuildSections]; }]];
        [a addAction:[UIAlertAction actionWithTitle:@"Clear BOOL force" style:UIAlertActionStyleDestructive handler:^(__unused UIAlertAction *act){ [FBTCSymbolStub setForce:nil forSymbol:entry.name]; [weakSelf rebuildSections]; }]];
    } else if (entry.function && [FBTCSymbolStub isTypedForceableSymbol:entry.name]) {
        NSString *kind = [FBTCSymbolStub returnKindForSymbol:entry.name] ?: @"typed";
        ...
        [a addAction:[UIAlertAction actionWithTitle:@"Observe only" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *act){ [FBTCSymbolStub setObserve:YES forSymbol:entry.name]; [weakSelf rebuildSections]; }]];
        [a addAction:[UIAlertAction actionWithTitle:@"Clear typed force" style:UIAlertActionStyleDestructive handler:^(__unused UIAlertAction *act){ [FBTCSymbolStub setTypedForceValue:nil returnKind:kind forSymbol:entry.name]; [weakSelf rebuildSections]; }]];
    } else if (entry.function && [FBTCSymbolStub isHookableSymbol:entry.name]) {
        [a addAction:[UIAlertAction actionWithTitle:@"Observe action/function" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *act){ [FBTCSymbolStub setObserve:YES forSymbol:entry.name]; [weakSelf rebuildSections]; }]];
    } else {
...
- (NSString *)entityKeyForEntry:(FBTCSymbolEntry *)entry {
    NSString *n = entry.name ?: @"";
...
- (NSArray<FBTCSymbolEntry *> *)controllableEntriesInGroup:(NSArray<FBTCSymbolEntry *> *)entries {
    NSMutableArray *out = [NSMutableArray array];
    for (FBTCSymbolEntry *e in entries ?: @[]) {
        FBTCRuntimePatchPlan *p = [self resolverPlanForEntry:e];
        if (p.inlineToggleSafe && p.strategy != FBTCRuntimePatchStrategyNone) [out addObject:e];
    }
    return out.copy;
}

- (NSString *)groupSubtitleForEntries:(NSArray<FBTCSymbolEntry *> *)entries {
    NSUInteger safe = 0, effective = 0, forced = 0, nativeOn = 0, hits = 0;
    for (FBTCSymbolEntry *e in entries ?: @[]) {
        FBTCRuntimePatchPlan *p = [self resolverPlanForEntry:e];
        if (p.inlineToggleSafe && p.strategy != FBTCRuntimePatchStrategyNone) safe++;
        id f = [FBTCRuntimePatchResolver currentForcedValueForPlan:p];
        id n = [FBTCRuntimePatchResolver currentNativeValueForPlan:p];
        if (f) forced++;
        if ([n isKindOfClass:NSNumber.class] && [n boolValue]) nativeOn++;
        if ([FBTCRuntimePatchResolver isEffectivelyEnabledForPlan:p]) effective++;
        hits += [FBTCRuntimePatchResolver hitCountForPlan:p];
    }
...
- (BOOL)groupSwitchValueForEntries:(NSArray<FBTCSymbolEntry *> *)entries {
    NSArray *safe = [self controllableEntriesInGroup:entries];
    if (!safe.count) return NO;
    for (FBTCSymbolEntry *e in safe) if (![self entrySwitchValue:e]) return NO;
    return YES;
}

- (UIView *)switchAccessoryForGroup:(FBTCSymbolGroup *)group {
    UISwitch *sw = [UISwitch new];
    sw.on = [self groupSwitchValueForEntries:group.entries];
    sw.onTintColor = [FBTUtils FBTColor_Primary];
    objc_setAssociatedObject(sw, &kFBTCSymbolGroupPayloadKey, group, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)runtimePatchGroupSwitchChanged:(UISwitch *)sender {
    FBTCSymbolGroup *group = objc_getAssociatedObject(sender, &kFBTCSymbolGroupPayloadKey);
    if (!group) return;
    NSArray *safe = [self controllableEntriesInGroup:group.entries];
    NSUInteger ok = 0;
    for (FBTCSymbolEntry *e in safe) {
        FBTCRuntimePatchPlan *p = [self resolverPlanForEntry:e];
        NSError *error = nil;
        BOOL success = NO;
        if (sender.isOn) success = [FBTCRuntimePatchResolver applyPlan:p value:(p.strategy == FBTCRuntimePatchStrategyFunctionObserve ? nil : @YES) error:&error];
        else {
            id native = [FBTCRuntimePatchResolver currentNativeValueForPlan:p];
            if ([native isKindOfClass:NSNumber.class] && [native boolValue] && (p.strategy == FBTCRuntimePatchStrategyObjCBool || p.strategy == FBTCRuntimePatchStrategyFunctionBool || p.strategy == FBTCRuntimePatchStrategyDataReaderBool)) {
                success = [FBTCRuntimePatchResolver applyPlan:p value:@NO error:&error];
            } else success = [FBTCRuntimePatchResolver revertPlan:p error:&error];
        }
        if (success) ok++;
    }
    [FBTUtils showToastForDuration:1.0 title:sender.isOn ? @"Group forced" : @"Group reverted/forced off" subtitle:[NSString stringWithFormat:@"%lu/%lu applied", (unsigned long)ok, (unsigned long)safe.count]];
    [self rebuildSections];
}
...
- (void)rebuildSections {
    ...
    NSMutableDictionary<NSString *, NSMutableArray<FBTCSymbolEntry *> *> *buckets = [NSMutableDictionary dictionary];

    for (FBTCSymbolEntry *e in _allSymbols) {
    ...
    for (NSString *header in order) {
        NSArray<FBTCSymbolEntry *> *entries = buckets[header] ?: @[];
        NSMutableArray<FBTBaseSettingsRow *> *rows = [NSMutableArray array];
        FBTCSymbolGroup *group = [FBTCSymbolGroup new];
        group.title = header;
        group.entries = entries;
        __weak typeof(self) weakSelf = self;
        FBTBaseSettingsRow *groupRow = [FBTBaseSettingsRow rowWithTitle:@"Stub / observe all safe entries" subtitle:nil action:nil];
        ...
        for (FBTCSymbolEntry *e in entries) {
            FBTBaseSettingsRow *row = [FBTBaseSettingsRow rowWithTitle:e.name subtitle:nil action:^(__unused UIViewController *vc){ [weakSelf pushRealtimeDetailForEntry:e]; }];
            ...
            objc_setAssociatedObject(row, &kFBTCSymbolRowPayloadKey, e, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
            [rows addObject:row];
        }
        [sections addObject:[FBTBaseSettingsSection sectionWithHeader:header footer:nil rows:rows]];
    }

    if (!sections.count) {
        [sections addObject:[FBTBaseSettingsSection sectionWithHeader:@"Runtime resolver" footer:@"Search another term or switch browser mode. Rows with a switch install the resolved hook immediately; tapping a row opens xref/disassembly and the full apply/revert screen." rows:@[
            [FBTBaseSettingsRow rowWithTitle:@"No matching runtime entries" subtitle:@"Search MobileConfig, EasyGating, Employee, Dogfood, SUBSBenefitDataProvider, ig_is_employee…" action:nil]
        ]]];
    } else {
        FBTBaseSettingsSection *last = sections.lastObject;
...
- (UIContextMenuConfiguration *)tableView:(UITableView *)tableView contextMenuConfigurationForRowAtIndexPath:(NSIndexPath *)indexPath point:(__unused CGPoint)point {
    FBTBaseSettingsRow *row = self.sections[indexPath.section].rows[indexPath.row];
    FBTCSymbolEntry *entry = objc_getAssociatedObject(row, &kFBTCSymbolRowPayloadKey);
    if (!entry) return nil;
    __weak typeof(self) weakSelf = self;
    return [UIContextMenuConfiguration configurationWithIdentifier:nil previewProvider:nil actionProvider:^UIMenu * _Nullable(__unused NSArray<UIMenuElement *> *suggestedActions) {
        ...
        if (entry.objcSelectorName.length) {
            ...
            [items addObject:[UIAction actionWithTitle:@"Force BOOL YES (hardstub)" image:[UIImage systemImageNamed:@"bolt.fill"] identifier:nil handler:^(__unused UIAction *action) { [FBTCSymbolStub setForce:@YES forSymbol:entry.name]; [weakSelf rebuildSections]; }]];
            [items addObject:[UIAction actionWithTitle:@"Clear BOOL force" image:[UIImage systemImageNamed:@"xmark.circle"] identifier:nil handler:^(__unused UIAction *action) { [FBTCSymbolStub setForce:nil forSymbol:entry.name]; [weakSelf rebuildSections]; }]];
        } else if (entry.function && [FBTCSymbolStub isTypedForceableSymbol:entry.name]) {
            NSString *kind = [FBTCSymbolStub returnKindForSymbol:entry.name] ?: @"typed";
            ...
            [items addObject:[UIAction actionWithTitle:@"Clear typed force" image:[UIImage systemImageNamed:@"xmark.circle"] identifier:nil handler:^(__unused UIAction *action) { [FBTCSymbolStub setTypedForceValue:nil returnKind:kind forSymbol:entry.name]; [weakSelf rebuildSections]; }]];
        } else if (entry.function && [FBTCSymbolStub isHookableSymbol:entry.name]) {
            [items addObject:[UIAction actionWithTitle:@"Observe action/function" image:[UIImage systemImageNamed:@"eye"] identifier:nil handler:^(__unused UIAction *action) { [FBTCSymbolStub setObserve:YES forSymbol:entry.name]; [weakSelf rebuildSections]; }]];
        }
